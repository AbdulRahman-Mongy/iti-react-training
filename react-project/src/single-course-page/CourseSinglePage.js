...
function CourseSinglePage() {
	...
	const attributes = {
		...
		students: "890,984",
		courses: "16",
		about: "You've taken a lengthy and boring coding class, but you're clueless about applying these concepts to build your ideas. The Codex shows you how to build fun, practical projects. Now, you can stand out from the crowd and land your dream job.",
	}

	useEffect(() => {
		axios
			.get("http://localhost:3000/singleCourse")
			.then((response) => {
				var data = response.data
				setData(data[courseID])
				setPending(false)
				setSections(
					data[courseID]["curriculum_context"]["data"]["sections"]
				)
			})
			.catch((error) => {
				setSections([])
	...
	const headerAttributes = {
		...
		ratings:
			course != undefined ? course["people"].slice(1) : "people ratings",
		students: "1,182",
		...
		return (
			<>
				...
				<div className="single-page-main-content">
					...
					<RequrimentList
						data={data ? data["details"]["Requirements"] : []}
					/>
